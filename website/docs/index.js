(async () => {
  ...
  const refreshTopicView = async () => {
    ...
    if (t) {
      ...
      if (topics.indexOf(t) != -1 && !topics[topics.indexOf(t) + 1]) {
        next.style.setProperty("display", "none");
      }
      next.addEventListener("click", () => {
        if (topics.indexOf(t) != -1) {
          if (topics[topics.indexOf(t) + 1]) {
            topics.map((x) => x.classList.remove("selected"));
            topics[topics.indexOf(t) + 1].click();
            topics[topics.indexOf(t) + 1].classList.add("selected");
            refreshTopicView();
          } else {
            next.style.setProperty("display", "none");
          }
