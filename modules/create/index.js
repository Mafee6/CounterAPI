    ...
    app.get("/api/create", (req, res) => {
        if (req.query.name) {
            if(req.query.name.length < 50) {
                const id = (Array.apply(0, new Array(10))).map(() => {
                    const c = Math.floor(Math.random() * 2);
                    if(c == 0) {
                        return Math.floor(Math.random() * 10)
                    } else {
                        return "abcdefghijklmnopqrstuvwxyz".split("")[Math.floor(Math.random() * 27)]
                    }
                }).join(""); 
            ...
            res.status(400).send(`Bad Request ${style}: Request Query Doesn't Contain a name`);
        }
    });  
};